import {drawRecordingFrame} from './recording';
...
export function PhysicalCodingLab(){
 ...
 useEffect(()=>{if(staticDeployment){setStatus('GitHub Pages showcase · local engine required');setIssue('The live FlyLab experiment uses a local Python service, Chromium workspace, and recorded neural data. Clone the repository and follow the Quick Start instructions to run it.');return;}let live=true;fetch('/api/physical-coding/catalog').then(r=>r.json()).then(c=>{if(live)setCatalog(c);}).catch(e=>setIssue(String(e)));connect();return()=>{live=false;socket.current?.close();stop.current=true;};
 // One dedicated browser connection per component; Start resets it explicitly.
 // eslint-disable-next-line react-hooks/exhaustive-deps
 },[]);
 useEffect(()=>{if(staticDeployment)return;fetch('/api/physical-coding/full-neuron-ids').then(r=>{if(!r.ok)throw new Error('Full CNS not prepared');return r.arrayBuffer();}).then(b=>setFullIds(Array.from(new Uint32Array(b),String))).catch(()=>{});},[]);
 ...
 const video=()=>{if(recording){recorder.current?.stop();setRecording(false);return;}const canvas=document.querySelector('.coding-world canvas') as HTMLCanvasElement;if(!canvas)return;const out=document.createElement('canvas');out.width=1600;out.height=1040;const ctx=out.getContext('2d')!;let raf=0;const render=()=>{drawRecordingFrame(ctx,{world:canvas,brain:document.querySelector('.coding-brain canvas') as HTMLCanvasElement|null,activity:document.querySelector('.cns-view canvas') as HTMLCanvasElement|null,screen,tick:latest.current.tick,status:latest.current.status});raf=requestAnimationFrame(render);};render();const stream=out.captureStream(30),media=new MediaRecorder(stream),chunks:BlobPart[]=[];recorder.current=media;media.ondataavailable=e=>chunks.push(e.data);media.onstop=async()=>{cancelAnimationFrame(raf);stream.getTracks().forEach(t=>t.stop());const r=await fetch('/api/coding/recording',{method:'POST',headers:{'Content-Type':'video/webm'},body:new Blob(chunks,{type:media.mimeType})});const saved=await r.json();setClip(saved.url);};media.start();setRecording(true);};
 ...
 {issue&&<div role="alert" className="coding-error">{issue}{staticDeployment&&<> <a href="https://github.com/Sudharsanselvaraj/The-Hawking-Fly#quick-start">Open Quick Start ↗</a></>}</div>}
 <footer className="coding-controls"><button className="coding-start" disabled={staticDeployment||running||!catalog?.manifest||(mode!=='subgraph'&&(!catalog?.full_network||!fullIds.length))} onClick={start}>Start</button>{running&&<button disabled={pausePending} onClick={togglePause}>{pausePending?'Pausing…':paused?'Resume':'Pause'}</button>}{running&&<button onClick={()=>{stop.current=true;pausedRef.current=false;setPaused(false);socket.current?.close();setRunning(false);setStatus('Stopped');}}>Stop</button>}
